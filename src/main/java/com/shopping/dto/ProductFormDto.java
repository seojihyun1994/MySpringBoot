...
@Getter @Setter @ToString // 상품 등록시 기입한 데이터 정보를 전달해주는 dto 클래스(command 객체)
public class ProductFormDto { // ProductFormDto01
    ...
    @NotNull(message = "재고 수량은은 필수 입력 항입니다.")
    private Integer stock ;
    private ProductStatus productStatus ;

    // 상품 등록시 첨부할 이미지 목록을(최대 5개) 저장할 리스트 컬렉션입니다.
    private List<ProductImageDto> productImageDtoList = new ArrayList<>() ;

    // 상품과 이미지들의 id 목록을 저장합니다.
    // 차후 수정시 참조할 id 입니다.
