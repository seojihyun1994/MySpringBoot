...
public class ProductImageService { // ProductImageService01
    ...
    public void saveProductImage(ProductImage productImage, MultipartFile uploadedFile) throws Exception { //ProductImageService01
        ...
        String imageUrl = "" ; // 상품 이미지를 읽어 들인 운영 체제 상의 경로(path)

        System.out.println("productImageLocation : " + productImageLocation);

        if(!StringUtils.isEmpty(oriImageName)){ // 원본 파일 이름이 존재하면 업로드 하자.
    ...
    public void updateProductImage(Long productImageId, MultipartFile uploadedFile) throws Exception {
        ...
        if(!uploadedFile.isEmpty()){
            // 이전 상품 정보를 읽어 옵니다.
            ProductImage previousImage = this.productImageRepository.findById(productImageId).orElseThrow(EntityExistsException::new) ;

            // 기존에 등록된 상품 이미지가 존재하면 해당 파일을 삭제 합니다.
            ...
            // 업데이트 하고자 하는 상품 이미지 파일을 업로드 합니다.
            ...
            String imageUrl = "/image/product/" + imageName ;
