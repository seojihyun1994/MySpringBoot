...
public class OrderController { // OrderController01
    ...
    @PostMapping(value = "/order") //@Valid 유효성 검사에 문제있으면 error로 들어감.
    public @ResponseBody ResponseEntity order(@RequestBody @Valid OrderDto orderDto, BindingResult error, Principal principal){
        ...
        //  SecurityConfig.java 파일과 연관 있음.
        //  .usernameParameter("email")
        String email = principal.getName();
        Long orderId ;

        try{
            // (상품 번호 + 수량 + 누가)를 가지고 주문을 합니다.
    ...
    public String orderHist(@PathVariable("page")Optional<Integer> page, Principal principal, Model model){
        ...
        // principal은 SecurityConfig usernameParameter 와 관계있음
        String email = principal.getName() ;
    ...
    // OrderController03)
    // 취소할 주문 번호는 조작이 될 수 있으므로, 주문 취소 권한 부터 검사하고 진행합니다.
