...
//    }
    ...
    public String productInsert(Model model){ // ProductController02
        model.addAttribute("productFormDto", new ProductFormDto()) ; // prodcutFormDto에 ProductFormDto객체를 담을게요.
        return "product/prInsertForm01" ;
    }

    // ProductController03
    // 파라미터 "productImageFile"가 넘겨준 데이터들을 productImageFileList에 저장해 줍니다.
    // List<MultipartFile>라고 한 이유는 <input type="file"> 이 5개 이기 때문입니다.
...
//        @RequestParam 파라미터를 챙길 수 있는 어노테이션
        ...
        try {
            ...
            err.printStackTrace();
            model.addAttribute("errMessage", "상품 등록 중 에러가 발생하였습니다.") ;
            return "product/prInsertForm01" ;
        }
    ...
    public String productDetail(@PathVariable("productId")Long productId, Model model){
        try{
            ...
            model.addAttribute("errorMessge", "존재 하지 않는 상품입니다.") ;
            model.addAttribute("productFormDto", new ProductFormDto()) ;
        }
    ...
    public String productUdate(
        ...
        // dto 에서 유효성검사 진행하는데 하나라도 에러가 있으면 hasErrors()에 들어있음
        if(error.hasErrors()){
            return whenError ;
        }
        ...
        try{
            ...
            model.addAttribute("errorMessage", "상품 수정 중에 오류가 발생 하였습니다.") ;
            err.printStackTrace();
            return whenError ;
        }
    ...
    public String productDetail(Model model, @PathVariable("productId") Long productId){
        // 메인 페이지에서 특정 상품을 클릭했을 때 상품 상세 페이지로 이동시키는 컨트롤러 메소드 입니다.
