...
public class ProductRepositoryCustomImpl implements ProductRepositoryCustom{
    ...
    private BooleanExpression dateRange(String searchDataType){
        // 사용자가 지정한 특정 기간 내의 데이터만 조회해주는 메소드 입니다.
    ...
    private BooleanExpression serchByCondition(String searchBy, String searchQuery){
        // searchBy) 조회할 컬럼 이름
    ...
    @Override // 검색 조건(dto)과 페이징 객체(pageble)를 이용하여 데이터를 조회합니다.
    public Page<Product> getAdminProductPage(ProductSearchDto dto, Pageable pageable) {
        QueryResults<Product> result = this.queryFactory
                ...
                .where(dateRange(dto.getSearchDataType()),
                       ...
                       serchByCondition(dto.getSearchby(), dto.getSearchQuery()))
        ...
        // Page 인터페이스의 구현체인 PageImpl 클래스에 대한 객체를 반환해 줍니다.  (Impl 이 있으면 구현체임)
        return new PageImpl<>(content, pageable, total);
    }

    // ProductRepositoryCustomImpl02
    // 상품과 상품 이미지 엔터티를 사용하여 상품 목록을 조회합니다.
    // 대표 이미지이고, 최신 이미지가 먼저 나오도록 조회합니다.
    @Override
